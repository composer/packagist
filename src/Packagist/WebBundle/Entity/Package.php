...
class Package
{
    ...
    public function isRepositoryValid(ExecutionContextInterface $context)
    {
        ...
        try {
            ...
            if (preg_match('{(free.*watch|watch.*free|movie.*free|free.*(movie|hack)|watch.*movie|watch.*full|generate.*resource|generate.*unlimited|hack.*coin|coin.*(hack|generat)|v[.-]?bucks|hack.*cheat|hack.*unlimited|cheat.*unlimited|cheat.*hack|putlocker|generat.*free|coins.*generat|(download|telecharg).*album|album.*(download|telecharg)|album.*(free|gratuit)|generat.*coins|unlimited.*coins|(fortnite|pubg|apex.*legend|t[1i]k.*t[o0]k).*(free|gratuit|generat|unlimited|coins|mobile|hack|follow))}i', $information['name'])) {
