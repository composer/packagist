...
class AuditLogDisplayFactory
{
    ...
    public function buildSingle(AuditRecord $record, bool $revealEmails = false): AuditLogDisplayInterface
    {
        return match ($record->type) {
            ...
            AuditRecordType::SecurityAdvisorySourceWithdrawn => new SecurityAdvisorySourceWithdrawnDisplay(
                $record->datetime,
                $record->attributes['name'],
                $record->attributes['advisoryId'],
                $record->attributes['cve'] ?? null,
                $record->attributes['title'],
                $record->attributes['source'],
                $record->attributes['remoteId'],
                $record->attributes['advisoryWithdrawn'] ?? false,
                $this->buildActor($record->attributes['actor'] ?? null),
                $record->ip,
            ),
            AuditRecordType::SecurityAdvisorySourceUnwithdrawn => new SecurityAdvisorySourceUnwithdrawnDisplay(
                $record->datetime,
                $record->attributes['name'],
                $record->attributes['advisoryId'],
                $record->attributes['cve'] ?? null,
